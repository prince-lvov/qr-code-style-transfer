...
from brevitas.quant import Int8ActPerTensorFloat, Uint8ActPerTensorFloat, Int8WeightPerTensorFloat, Int32Bias, Int16Bias, Int8Bias, NoneBiasQuant
from brevitas.quant_tensor import QuantTensor


class QuantAEPruned(nn.Module):
    ...
    def prune(self, enable):
        n_active = 6
        
        conv_layers = [self.encoder[1], self.encoder[5], self.encoder[9]]
        ...
        if enable:
            prune.l1_unstructured(
                self.decoder_input, 'weight', amount=(self.latent_dim - n_active) * self.decoder_dims[0]
            )
            prune.l1_unstructured(
                self.decoder[0], 'weight', amount=(self.decoder_dims[0] - n_active) * self.decoder_dims[1]
            )
        else:
            prune.remove(self.decoder_input, 'weight')
            prune.remove(self.decoder[0], 'weight')
    ...
    def forward(self, input: Union[Tensor, QuantTensor], **kwargs) -> Union[Tensor, QuantTensor]:
        #prune.random_unstructured(self.encoder[1], name="weight", amount=0.3)
